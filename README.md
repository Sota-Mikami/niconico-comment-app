...
| ファイル名 | 対象 Mac |
|---|---|
| `コメントアプリ-x.x.x-arm64.dmg` | Apple Silicon (M1 / M2 / M3) |
| `コメントアプリ-x.x.x.dmg` | Intel Mac |
...
2. 「コメントアプリ」を `Applications` フォルダへドラッグ＆ドロップ
3. **初回起動時のみ**: `Applications` フォルダで右クリック → **「開く」** → 警告ダイアログで **「開く」** をクリック

> ⚠️ Apple の公証を受けていないため、初回のみ右クリック→開くが必要です。2回目以降は通常通り起動できます。
...
| 「開発元を確認できません」が出る | 右クリック → 開く → ダイアログで「開く」 |
