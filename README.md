...
3. **初回のみ**: アプリをダブルクリック → ブロックされたら **システム設定 → プライバシーとセキュリティ** →「このまま開く」をクリック

> ⚠️ Apple の公証を受けていないため、初回のみ上記の許可操作が必要です。2回目以降は通常通り起動できます。
...
| 「Apple could not verify…」が出る | システム設定 → プライバシーとセキュリティ →「このまま開く」 |
